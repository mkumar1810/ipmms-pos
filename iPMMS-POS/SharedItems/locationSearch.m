         ...
         andIsSplit:(BOOL) p_issplitmode
{
    ...
    if (self) {
        ...
        //[[NSNotifxxicationCenter defaultCenter] addObserver:self selector:@selector(fireCancelNotification:) name:@"fireCancelLocationSelect" object:nil];
        ...
        if (isSplitMode) 
        {
            ...
            //[[NSNxxotificationCenter defaultCenter] postNotificationName:@"navigationNotify" object:self userInfo:naviInfo];
            _returnMethod(naviInfo);
        }
        else
        {
            navBar.hidden =NO;
...
- (id)initWithFrame:(CGRect)frame forOrientation:(UIInterfaceOrientation) p_intOrientation andNotification:(NSString*) p_notification withNewDataNotification:(NSString*)  p_proxynotificationname andIsSplit:(BOOL) p_issplitmode
{
    ...
    if (self) {
        ...
        _cacheName = [[NSString alloc] initWithString:@"ALLLOCATIONS"];
        _gobacknotifyName = [[NSString alloc] initWithFormat:@"%@",p_notification];
        //[[NSNotificxxationCenter defaultCenter] addObserver:self selector:@selector(locationListDataGenerated:)  name:_proxynotification object:nil];
        //[[NSNotificxxationCenter defaultCenter] addObserver:self selector:@selector(fireCancelNotification:) name:@"fireCancelLocationSelect" object:nil];
        _notificationName = [[NSString alloc] initWithFormat:@"%@",p_notification];
        [actIndicator startAnimating];
        sBar.text = @"";
        sBar.hidden = YES;
        if (isSplitMode) 
        {
            ...
            NSDictionary *buttonData = [[NSDictionary alloc] initWithObjectsAndKeys:@"Cancel", @"btntitle",@"fireCancelLocationSelect",@"btnnotification" ,  nil];
            NSDictionary *naviInfo = [[NSDictionary alloc] initWithObjectsAndKeys:buttonData,[NSString stringWithFormat:@"%d",101] , @"Select a Location",@"navititle" ,nil];
            //[[NSNotificatioxxnCenter defaultCenter] postNotificationName:@"navigationNotify" object:self userInfo:naviInfo];
            _returnMethod(naviInfo);
        }
        else
        {
...
- (void) generateData
{
    if (populationOnProgress==NO)
    {
        ...
        posWSCorecall = [[posWSProxy alloc] initWithReportType:_webdataName andInputParams:nil andResponseMethod:_wsReturnMethod];
...
- (void) locationListDataGenerated:(NSDictionary *)generatedInfo
{
    ...
    /*if (!isSplitMode) 
    {
        ...
        [[NSNotificatixxonCenter defaultCenter] postNotificationName:_notificationName object:self userInfo:returnInfo];
    }*/
}
